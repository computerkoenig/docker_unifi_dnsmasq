...
def get_configured_clients(session):
    ...
    r = session.get(baseurl + '/api/s/' + site + '/list/user', verify=False)
    r.raise_for_status()
    return r.json()['data']
...
def get_active_clients(session):
    # Get active clients
    r = session.get(baseurl + '/api/s/' + site + '/stat/sta', verify=False)
    r.raise_for_status()
    return r.json()['data']
...
def get_clients():
    ...
    r = session.post(baseurl + '/api/login', json={'username': username, 'password': password}, verify=False)
    r.raise_for_status()
